...
int main( void )
{
    ...
    const char psk[]={0x33,0x44,0x55};
    //const char *psk_identity="Client_identity";
    const char *psk_identity="18602560533";
	char *psk_list = NULL;
    //size_t psk_len = 0;
     ...
     */
    printf( "  . Bind on udp/*/5684 ..." );
    fflush( stdout );

    if( ( ret = mbedtls_net_bind( &listen_fd, NULL, "5684", MBEDTLS_NET_PROTO_UDP ) ) != 0 )
    {
...
#if defined(MBEDTLS_KEY_EXCHANGE__SOME__PSK_ENABLED)
#if 1
    if(sizeof(psk)!=0 && strlen(psk_identity)!=0)
    {
        ret = mbedtls_ssl_conf_psk(&conf,(const unsigned char *)psk,sizeof(psk),
			(const unsigned char *)psk_identity,strlen(psk_identity));
		if(ret!=0)
		{
		    printf("failed mbedtls_ssl_conf_psk\n");
		    goto exit;
		}
		if(psk_list!=NULL)
