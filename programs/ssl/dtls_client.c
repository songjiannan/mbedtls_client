...
#define SERVER_PORT "5685"
#define SERVER_NAME "fire"
#define SERVER_ADDR "192.168.1.116" /* forces IPv4 */
...
{
    char psk[] = "11223344556677881122334455667788";
    char psk_identity[] = "testserver1";
    if ((ret = mbedtls_ssl_conf_psk(&conf, (const unsigned char *)psk, strlen(psk),
                                    (const unsigned char *) psk_identity,
                                    strlen(psk_identity))) != 0)
    {
        mbedtls_printf("mbedtls_ssl_conf_psk failed: -0x%x\n\n", -ret);
        goto exit;
    }
}
#endif
...
//    {
 //       MBEDTLS_LOG( " failed\n  ! mbedtls_ssl_cookie_setup returned %X\n\n", -ret );
 //       goto exit;
 //   }
